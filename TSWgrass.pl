...
# in SWhybpipe1.pl and SWhybpipe3.pl. 
#
# Call as: perl TSWgrass.pl datafile-at-the-fnamematrix-stage target max_time
#
# Note that TSWgrass.pl takes in a datamatrix, and not a Newick-format specification of trees, unlike SWgrass.pl.
#
# Note also, that
...
#
system ("rm tswtrace");
print "\n\n Starting TSW with input matrix:\n";
  ...
  #print "The file name (passed to TSW from (SWhybrid.pl, or Rdecomp, or GRdecomp, or at the direct call) is $ARGV[0] \n";
...
system ("perl clean_no_cluster.pl $dm"); # DG Dec. 10, 2020 call the version of clean_for_hybridization that does not do the find-cluster computation,
                                         # which has already been done when TSWgrass.pl is called from Rdecomp or GRdecomp.
					 # 
					 #
					 #
#system ("perl clean_for_hybridtrees.pl $dm"); # when TSWgrass.pl is called from Rdecomp or GRdecomp, the input file passed to it should already
                                              # be known not to have any subclusters, so this call to clean_for_hybridtrees.pl is superfluous, and may
					      # cause confusion in the output because it's execution will again report that there are no subclusters.
                                        
$cdata  = $dm . 'clean';
#print "The file after clean_for_hybridtrees is: $cdata \n";
$gtimediff = 0;

# $interior_num = int (($n - 2 + 2 * $target) * 0.65); # now compute interior_num in Thybridnum.pl  # This is not used when TSWgrass is used together with SWgrass. Instead
#                                                                                                   # interior_num is set in Thybridnum, which is called from TSWgrass.
...
print "The total time used in SAT instances is $total_sat_time \n";
##########
...
$sat = 1;
 ...
 while ($sat)  { 
  ...
  print "The total time used by the solver is $totalsearchtime \n";
...
#       break;
    ...
    if ($line =~ /INDETERMINATE/) {
       ...
       print "target value $target is INDETERMINATE with cryptominisat5 \n";
...
sub cleanup {  # Note that in TSWgrass.pl cleanup is not called, and cleanup is the only place where remove_dup_columns is called.
